...
BEGIN
    ...
    -- TEST K: Rejected -> Active by Staff (Expect DENY / 0 rows updated / status remains Rejected)
    PERFORM set_config('request.jwt.claims', '{"sub": "33333333-3333-3333-3333-333333333333", "role": "authenticated"}', true);
    BEGIN
        UPDATE public.connected_devices SET status = 'Active' WHERE device_id = 'DEV_STAFF_A';
    EXCEPTION WHEN OTHERS THEN
    ...
    END;
    SELECT status INTO rec FROM public.connected_devices WHERE device_id = 'DEV_STAFF_A';
    IF rec.status != 'Rejected' THEN RAISE EXCEPTION 'TEST K FAILED: Rejected device became %', rec.status; END IF;

    -- TEST L: Rejected -> Pending by Staff (Expect DENY / 0 rows updated / status remains Rejected)
    BEGIN
        UPDATE public.connected_devices SET status = 'Pending' WHERE device_id = 'DEV_STAFF_A';
    EXCEPTION WHEN OTHERS THEN
        NULL;
    END;
    SELECT status INTO rec FROM public.connected_devices WHERE device_id = 'DEV_STAFF_A';
    IF rec.status != 'Rejected' THEN RAISE EXCEPTION 'TEST L FAILED: Rejected device became %', rec.status; END IF;
