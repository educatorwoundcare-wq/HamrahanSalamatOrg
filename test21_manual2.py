...
print(run_query(f"""
BEGIN;
INSERT INTO auth.users (id) VALUES ('{UID_A}'), ('{UID_E}') ON CONFLICT DO NOTHING;
...
INSERT INTO connected_devices (company_id, device_id, uid, role, status) VALUES ('{COMP_A}', 'DEV_E', '{UID_E}', 'Mother Account', 'Inactive');
COMMIT;
"""))
...
print("UID_E as role:")
print(run_query(as_role(UID_E, f"SELECT count(*) FROM workspaces WHERE company_id = '{COMP_A}';")))
print("is_device_member test:")
print(run_query(as_role(UID_E, f"SELECT is_device_member_of_workspace('{COMP_A}');")))
