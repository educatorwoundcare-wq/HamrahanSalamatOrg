...
ok, res = run_query(as_role(UID_A, f"SELECT count(*) as c FROM workspaces WHERE company_id = '{COMP_A}';"))
c = res[-1]['c'] if (ok and isinstance(res, list) and len(res)>0 and 'c' in res[-1]) else -1
if c == 0:
...
run_query(f"DELETE FROM auth.users WHERE id IN ('{UID_A}', '{UID_B}', '{UID_C}', '{UID_NODEV}');")
